    ...
    <div {...getRootProps()}
      className={`relative border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all duration-200 select-none
        ${isDragActive
          ...
          : 'border-slate-700 bg-dark-600 hover:border-blue-500/50 hover:bg-blue-500/5'}
        ${disabled ? 'opacity-40 cursor-not-allowed' : ''}`}>
      <input {...getInputProps()} />
      <div className="flex flex-col items-center gap-3">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl transition-all
          ${isDragActive ? 'bg-blue-500/20 scale-110' : 'bg-slate-800 border border-slate-700'}`}>
          {isDragActive ? '📂' : '🫁'}
        </div>
        <div>
        ...
        <div className="flex items-center gap-2 text-xs text-slate-600 bg-dark-700 border border-slate-800 px-3 py-1.5 rounded-full">
