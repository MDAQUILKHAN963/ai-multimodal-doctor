...
const CONDITION_STYLE = {
  'COVID':        { bg: 'bg-rose-500/15',   border: 'border-rose-500/30',   text: 'text-rose-600',   dot: 'bg-rose-500'    },
  'Pneumonia':    { bg: 'bg-amber-500/15',  border: 'border-amber-500/30',  text: 'text-amber-600',  dot: 'bg-amber-500'   },
  'Lung Opacity': { bg: 'bg-orange-500/15', border: 'border-orange-500/30', text: 'text-orange-600', dot: 'bg-orange-500'  },
  'Normal':       { bg: 'bg-emerald-500/15',border: 'border-emerald-500/30',text: 'text-emerald-600',dot: 'bg-emerald-500' },
};
const DEFAULT_STYLE = { bg: 'bg-blue-500/15', border: 'border-blue-500/30', text: 'text-blue-600', dot: 'bg-blue-500' };
  ...
  return (
    <div className="card p-6 space-y-5 animate-slide-up">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="text-xs text-slate-600 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200">
          {result.model}
        </span>
      </div>
      ...
      {result.heatmap_base64 && (
        <div>
          ...
          <img src={`data:image/png;base64,${result.heatmap_base64}`} alt="Grad-CAM"
            className="rounded-xl border border-slate-200 w-full max-w-sm object-contain bg-slate-50" />
        </div>
      )}
      ...
      {result.gemini_explanation && (
        <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-bold text-blue-700">AI Explanation</p>
          </div>
          <div className="text-sm text-slate-700 leading-relaxed prose prose-sm prose-invert max-w-none">
